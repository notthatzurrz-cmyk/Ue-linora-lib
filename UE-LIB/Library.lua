...
local Library = {
    ...
    FontColor = Color3.fromRGB(245, 247, 250);
    MainColor = Color3.fromRGB(10, 10, 12);
    BackgroundColor = Color3.fromRGB(6, 6, 8);
    AccentColor = Color3.fromRGB(56, 140, 255);
    OutlineColor = Color3.fromRGB(24, 26, 32);
...
function Library:GetTextBounds(Text, Font, Size, Resolution)
    local useFont = Font
    if typeof(useFont) == 'Font' then
        useFont = Enum.Font.Code
    end
    local Bounds = TextService:GetTextSize(Text, Size, useFont, Resolution or Vector2.new(1920, 1080))
...
function Library:CreateWindow(...)
    ...
    if typeof(Config.Size) ~= 'UDim2' then Config.Size = UDim2.fromOffset(530, 680) end
    ...
    function Window:AddTab(Name)
        ...
        function Tab:ShowTab()
            for _, Tab in next, Window.Tabs do
                if type(Tab) == 'table' and type(Tab.HideTab) == 'function' then
                    Tab:HideTab();
                end;
            end;
    ...
    function Library:Toggle()
        ...
        if Library.Toggled then
            ...
            for _, t in next, Window.Tabs do
                if type(t) == 'table' and t.TabFrame and t.TabFrame.Visible then
                    hasVisibleTab = true;
                    break;
                end;
            end;
            if not hasVisibleTab then
                local defaultTab = Window.Tabs['Aimbot'] or Window.Tabs['ESP'] or next(Window.Tabs);
                if defaultTab and type(defaultTab.ShowTab) == 'function' then
                    defaultTab:ShowTab();
                end;
            end;
            task.spawn(function()
